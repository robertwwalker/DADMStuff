...
Final.Data <- left_join(kids, State.Temp, by = c('state' = 'name')) %>% filter(variable.x == "highered")
ggplot(Final.Data, aes(x=year, y=inf_adj_perchild, color = inf_adj_perchild)) +
  geom_line() +
  theme_void() +
  scale_color_viridis_c() +
  facet_geo(~ state.y) + labs("Pre-K through 12 Education Spending per Child", subtitle="Inflation adjusted", fill="Spend per Child") -> My.Plot
My.Plot
```
